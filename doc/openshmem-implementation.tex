...
variable waits (\emph{e.g.} shmem\_long\_waituntil) to poll GASNet,
...
terms. A simple extension could \emph{e.g.} just multiply the distance
...
\texttt{complex<T>}.  The use of complex number routines (\emph{e.g.}
...
(\emph{e.g.}  GNU, Intel) and links against the SMA and MPI libraries.
...
operate in threaded environments, \emph{e.g.} alongside OpenMP;
