...
//    float (*d)[n];
//    d = (float (*) [n])malloc( 3*sizeof(*d) );

// X-1 
#define FILENAME "tmp.image"

// X-3
void itstep( int mx, int my, void *pf, void *pnewf, void *pr, double rdx2, double rdy2, double beta )
{
...
// end change here.
/***/
       extern void itstep( int mx, int my, void *f, void *newf, void *r, double rdx2, double rdy2, double beta );
...
/* Do all the transfers: */
	  shmem_barrier_all();
	  if ( my_number > 0 )              shmem_double_put( &(newf[partmx-1][1]), &(newf[1][1]),    my-2, my_number-1 );
	  if ( my_number < (n_of_nodes-1) ) shmem_double_put( &(newf[0][1]),        &(newf[mx-2][1]), my-2, my_number+1 );
	  shmem_barrier_all();
/* swap the halves: */	  
	  curf = 1 - curf;
	 }
