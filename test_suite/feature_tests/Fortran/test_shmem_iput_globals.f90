! (c) 2011 University of Houston System.  All rights reserved. */
! 
! Calls tested
! shmem_integer_iput, shmem_real_put, shmem_double_put
! shmem_logical_iput, shmem_character_put 
! shmem_iput32, shmem_iput64
! 
! All PEs put an array on the right neighbor 
!  
program test_shmem_iput
  implicit none
  include 'mpp/shmem.fh'

  integer, parameter :: N = 7

  integer                 ::  i,j
  integer                 ::  nextpe
  integer                 ::  me, npes
  integer                 ::  success1,success2,success3, success4, success5, success6

  integer          , save :: dest1(N)
  real             , save :: dest2(N)
  double precision , save :: dest3(N)
  character        , save :: dest4(N)
  character        , save :: dest5(N)
  logical          , save :: dest6(N)
  integer*8        , save :: dest7(N)

  integer                 :: src1(N)
  real                    :: src2(N)
  double precision        :: src3(N)
  character               :: src4(N)
  character               :: src5(N)
  logical                 :: src6(N)
  integer*8               :: src7(N)

  integer                 :: length, errcode, abort

! Function definitions
  integer                   :: my_pe, num_pes  

  call start_pes(0)
  me   = my_pe();
  npes = num_pes();

! Make sure this job is running on at least 2 PEs
  if(npes .gt. 1) then

    success1 = 0
    success2 = 0
    success3 = 0
    success4 = 0
    success5 = 0
    success6 = 0

    length = N

    do i = 1, N, 1
      dest1(i) = -9
      dest2(i) = real(9)
      dest3(i) = dble(9)
      dest4(i) = char(9)
      dest5(i) = char(9)      
      dest6(i) = .false.
      dest7(i) = int(-9, KIND=8)
    end do 

    do i = 1, N, 1
      src1(i) = me
      src2(i) = real(me)
      src3(i) = dble(me)
      src4(i) = char(me)
      src5(i) = char(me)
      src6(i) = .true.
      src7(i) = int(me, KIND=8)
    end do 

    nextpe = mod((me + 1), npes)

    call shmem_barrier_all()

    call shmem_integer_iput(dest1, src1, 1, 2, N, nextpe)
    call shmem_real_iput(dest2, src2, 1, 2, N, nextpe)
    call shmem_double_iput(dest3, src3, 1, 2, N, nextpe)
    call shmem_logical_iput(dest6, src6, 1, 2, N, nextpe)

    call shmem_barrier_all()

    if(me .eq. 0) then
      do i = 0, N/2, 1
        if(dest1(i + 1) .ne. (npes - 1)) then
          success1 = success1 + 1
        end if
        if(dest2(i + 1) .ne. (npes - 1)) then
          success2 = success2 + 1
        end if
        if(dest3(i + 1) .ne. (npes - 1)) then
          success3 = success3 + 1
        end if
        if(dest5(i + 1) .ne. char(npes - 1)) then
          success5 = success5 + 1
        end if
        if(.not. dest6(i + 1)) then
          success6 = success6 + 1
        end if
      end do 

      if(success1 .eq. 0) then
        write(*,*) "Test shmem_integer_iput: Passed" 
      else
        write(*,*) "Test shmem_integer_iput: Failed"
      end if
      if(success2 .eq. 0) then
        write(*,*) "Test shmem_real_iput: Passed"  
      else
        write(*,*) "Test shmem_real_iput: Failed"
      end if
      if(success3 .eq. 0) then
        write(*,*) "Test shmem_double_iput: Passed"  
      else
        write(*,*) "Test shmem_double_iput: Failed"
      end if
      if(success6 .eq. 0) then
        write(*,*) "Test shmem_logical_iput: Passed"  
      else
        write(*,*) "Test shmem_logical_iput: Failed"
      end if
    end if 

    call shmem_barrier_all()

    ! Testing shmem_iput32, shmem_put64, shmem_put128 
    do i = 1, N, 1
      dest1(i) = -9
      dest7(i) = -9
    end do 

    success1 = 0

    call shmem_barrier_all()

    call shmem_iput32(dest1, src1, 1, 2, N, nextpe)
    call shmem_iput64(dest7, src7, 1, 2, N, nextpe)

    call shmem_barrier_all()

    if(me .eq. 0) then
      do i = 0, N/2, 1
        if(dest2(i + 1) .ne. npes - 1) then
          success2 = success2 + 1
        end if
        if(dest7(i + 1) .ne. npes - 1) then
          success3 = success3 + 1
        end if
      end do

      if(success2 .eq. 0) then
        write(*,*) "Test shmem_iput32: Passed"  
      else
        write(*,*) "Test shmem_iput32: Failed"
      end if

      if(success3 .eq. 0) then
        write(*,*) "Test shmem_iput64: Passed" 
      else
        write(*,*) "Test shmem_iput64: Failed"
      end if

    end if

    call shmem_barrier_all()

  else
    write(*,*) "Number of PEs must be > 1 to test shmem get, test skipped"
  end if
end program
