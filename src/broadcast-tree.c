...
/*************************************************
Tree based broadcast generates a binary tree with the PEs in the active with PE_root as the root.
The puts happen in a top down approach.
*************************************************/


void
set_2tree(int PE_start, int PE_stride, int PE_size,int* parent, int* child_l, int* child_r, int my_pe)
{
	...
	*child_r = *child_l + PE_stride;
	...
	if (*child_l > max_pe) *child_l = *child_r = -1;
	else if (*child_r > max_pe) *child_r = -1;
	//printf("In set2tree:For PE %d   Parent = %d, Lchild = %d, Rchild = %d", my_pe,*parent, *child_l, *child_r);
}


void
build_tree(int PE_start,int step,int PE_root,int PE_size,int* parent,int* child_l,int* child_r, int my_pe)
{ 
	int inter;
	if (PE_root != 0) {
		int PE_root_abs;
		...
		if (__state.mype == PE_start) {
			set_2tree(PE_start, step, PE_size, &other_parent, &other_child_l, &other_child_r, PE_root_abs);

			if (other_parent == __state.mype) *parent = PE_root_abs;
			else *parent = other_parent;
		...
		else if (__state.mype == PE_root_abs) {

			set_2tree(PE_start, step, PE_size, &other_parent,&other_child_l, &other_child_r, PE_start);
		...
		else if (*parent == PE_start) *parent = PE_root_abs;
		else if (*parent == PE_root_abs) *parent = PE_start;

		/* update potential parents of exchanged nodes */
		if(__state.mype != PE_start) {
			if (*child_l == PE_root_abs) *child_l = PE_start;
			else if (*child_r == PE_root_abs) *child_r = PE_start;
		}
	}
}


void pshmem_broadcast32_tree(void *target, const void *source, size_t nlong,int PE_root, int PE_start, int logPE_stride, int PE_size,long *pSync)				
{															
	int child_l, child_r, parent;						
	const int step = 1 << logPE_stride;							
	int my_pe = __state.mype;									
	int *target_ptr, *source_ptr;
	int old_target;

	target_ptr = (int *) target;
	source_ptr = (int *) source;
	set_2tree(PE_start, step, PE_size, &parent,&child_l, &child_r, __state.mype);			

	old_target = *target_ptr;

	build_tree(PE_start, step, PE_root,PE_size, &parent, &child_l, &child_r, __state.mype);
...
void pshmem_broadcast64_tree(void *target, const void *source, size_t nlong,int PE_root, int PE_start, int logPE_stride, int PE_size,long *pSync)
{
	int child_l, child_r, parent;
	const int step = 1 << logPE_stride;
	int inter;
