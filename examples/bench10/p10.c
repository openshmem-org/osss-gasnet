...
long int *TSOLS; // TONY: shmalloc for gatech
...
unsigned long *totalSolutions; // TONY: needs to be shmalloced for gatech code
...
void p10 (int WDTH, int *NSOLS)
{
   ...
   totalSolutions = (unsigned long *)shmalloc(sizeof(*totalSolutions));
assert(totalSolutions != (unsigned long *)NULL);
   TSOLS = (long *)shmalloc(MAXTS * sizeof(*TSOLS));
assert(TSOLS != (long *)NULL);
   step = (unsigned long *)shmalloc(sizeof(*step));
assert(step != (unsigned long *)NULL);
